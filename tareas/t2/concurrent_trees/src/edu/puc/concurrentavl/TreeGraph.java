...
public class TreeGraph extends JFrame
{
    ...
    {
        if (N==null) return 0;
        int l = depth(N.getChild(N.LEFT));
        int r = depth(N.getChild(N.RIGHT));
    ...
    private void drawNode(Node N,int l, int lb, int rb)
    {
        ...
        if (N.getChild(N.LEFT)!=null)
        {
            display.drawLine((lb+rb)/2,yoff+10+(l*bheight),
                    ((3*lb+rb)/4),yoff+(l*bheight+bheight));
            drawNode(N.getChild(N.LEFT),l+1,lb,(lb+rb)/2);
        }
        if (N.getChild(N.RIGHT)!=null)
        {
            display.drawLine((lb+rb)/2,yoff+10+(l*bheight),
                    ((3*rb+lb)/4),yoff+(l*bheight+bheight));
            drawNode(N.getChild(N.RIGHT),l+1,(lb+rb)/2,rb);
        }
    } // drawnode
